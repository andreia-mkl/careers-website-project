<template>
  <li class="mb-7">
    <router-link
      :to="jobPageLink"
      class="mx-auto block rounded border border-solid border-brand-grey-2 bg-white hover:shadow-grey"
    >
      <div class="mx-8 border-b border-solid border-brand-grey-2 pb-2 pt-5">
        <h2 class="mb-2 text-2xl">{{ job.title }} / Superstar</h2>
        <div class="flex flex-row align-middle">
          <div class="mr-5">{{ job.organization }}</div>
        </div>
        <div class="">
          <ul>
            <li v-for="location in job.locations" :key="location" class="mr-5 inline-block">
              {{ location }}
            </li>
          </ul>
        </div>
      </div>
      <div class="px-8 py-4">
        <div>
          <h3 class="mb-2 mt-1">
            <ul class="list-disc pl-8">
              <li v-for="qualification in job.minimumQualifications" :key="qualification">
                {{ qualification }}
              </li>
            </ul>
          </h3>
        </div>
        <div class="mt-2 text-center">
          <router-link :to="jobPageLink" class="text-brand-blue-1"> Expand </router-link>
        </div>
      </div>
    </router-link>
  </li>
</template>

<script lang="ts" setup>
import { computed, type PropType } from 'vue';

import type { Job } from '@/api/types';

const props = defineProps({
  job: {
    type: Object as PropType<Job>,
    required: true
  }
});

const jobPageLink = computed(() => `/jobs/results/${props.job.id}`);

// export default {
//   name: 'JobListing',
//   props: {
//     job: {
//       type: Object,
//       required: true
//     }
//   },
//   computed: {
//     jobPageLink() {
//       return `/jobs/results/${this.job.id}`;
//     }
//   }
// };
</script>
